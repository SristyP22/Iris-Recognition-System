 ...
 * NOTE: source obtained from public java.sun.com forum
 */

import java.awt.*;
import java.awt.image.*;

public class CannyEdgeDetector extends Component
{
	protected final float ORIENT_SCALE = 40F;
	protected int height;
	protected int width;
	protected int picsize;
	protected int data[];
	protected int derivative_mag[];
	protected int magnitude[];
	protected int orientation[];
	protected Image sourceImage;
	protected GrayscaleImage edgeImage;
	protected int threshold1;
	protected int threshold2;
	protected int threshold;
	protected int widGaussianKernel;

	public CannyEdgeDetector()
	{
		this.threshold1 = 30;
		this.threshold2 = 180;
		this.setThreshold(Globals.EDGE_THRESHHOLD);
		this.setWidGaussianKernel(Globals.GUASSIAN_WIDTH);
	}
	...
	public void process() throws EdgeDetectorException
	{
		...
		this.width = sourceImage.getWidth(this);
		this.height = sourceImage.getHeight(this);
		this.picsize = width * height;
		this.data = new int[picsize];
		this.magnitude = new int[picsize];
		this.orientation = new int[picsize];

		float f = 1.0F;

		this.canny_core(f, widGaussianKernel);
		...
		this.edgeImage = new GrayscaleImage(pixels2image(data));
		this.data = null;
		this.magnitude = null;
		this.orientation = null;
	}

	protected void canny_core(float f, int i)
	{
		boolean flag = false;
		boolean flag1 = false;
	...
	protected float hypotenuse(float f, float f1)
	{
		if (f == 0.0F && f1 == 0.0F)
			return 0.0F;
		else
	...
	protected float gaussian(float f, float f1)
	{
	    float xxx = (float) Math.exp((-f * f) / ((float) 2 * f1 * f1));

		return (float) Math.exp((-f * f) / ((float) 2 * f1 * f1));
	}

	protected void thresholding_tracker(int i, int j)
	{
	...
	protected boolean follow(int i, int j, int k)
	{
		int j1 = i + 1;
		int k1 = i - 1;
		int l1 = j + 1;
	...
	protected Image pixels2image(int ai[])
	{
	...
	protected int[] image2pixels(Image image)
	{
	...
	public void setSourceImage(MedianImage img)
	{
		sourceImage = img.getImage();
	}
...
class EdgeDetectorException extends Exception
{
  ...
  public EdgeDetectorException(String s) { super(s); }
}